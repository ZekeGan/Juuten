...
const Main = styled.div`
  ...
  transform: translateY(
    ${(p) =>
    p.open ? (p.fullPage ? `-${p.config.max_height}px` : "-400px") : "0"}
  );
  ...
  ({
    ...
    return (
      <Main config={configuration} open={open} fullPage={fullPage}>
        ...
        {useContainer
          && (<Container config={configuration}>{children}</Container>)}

        {!useContainer && children}
      </Main>
    );
  },
  (prev, next) => {
    if (prev.open || next.open) return prev === next;
    return true;
  }
);
